...
{
	uint8_t tempReg = I2C_RdReg(reg,rxBuf,1);
	if(set)
	{
		//set bits
		tempReg |= bitmask;
...
//we only use the interrupt line 1
void mma8653_initIrq(IRQTypes irqType)
{
	...
	I2C_BitHandling(REG_CTRL_REG_4,1<<irqType,1);
	//and route it to the corresponding pin
	I2C_BitHandling(REG_CTRL_REG_5,1<<irqType,0);

	//TODO: set up the Interrupt in the STM32!
...
//TODO: don't forget to register the interrupt handler in startup code
