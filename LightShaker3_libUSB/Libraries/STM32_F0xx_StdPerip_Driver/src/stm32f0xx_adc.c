...
void ADC_Init(ADC_TypeDef *ADCx, ADC_InitTypeDef *ADC_InitStruct) {
	...
	assert_param(
			IS_ADC_EXTERNAL_TRIG_CONV(ADC_InitStruct->ADC_ExternalTrigConv));
	...
	tmpreg |=
			(uint32_t) (ADC_InitStruct->ADC_Resolution
					| ((uint32_t) (ADC_InitStruct->ADC_ContinuousConvMode) << 13)
...
ITStatus ADC_GetITStatus(ADC_TypeDef *ADCx, uint32_t ADC_IT) {
	...
	if (((uint32_t) (ADCx->ISR & ADC_IT) != (uint32_t) RESET)
